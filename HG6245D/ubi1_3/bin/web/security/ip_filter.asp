...
/*add begin by 陈鹏 20180730 原因:增加IPV4过滤页面源地址、目的地址的输入校验 需求来源:龚忠锋*/
function fw_checkIP2(domStart, domEnd)
{	
	domStart.value = trim(domStart.value);
	domEnd.value = trim(domEnd.value);

	if(!CheckNotNull(domEnd.value))	//结束地址为空
	{
		return true;
	}	
	if(!CheckNotNull(domStart.value) && CheckNotNull(domEnd.value))	//结束地址不为空时，起始地址为空
	{
		alert(_("fw_StartIPNullAlert"));
		return false;
	}
	//起始地址、结束地址均不为空，起始地址大于结束地址
	if(CheckNotNull(domStart.value) && CheckNotNull(domEnd.value) && (IpAddress2DecNum(domEnd.value) - IpAddress2DecNum(domStart.value) < 0))	
	{
...
//	if(confirm("你确认删除所选规则?") == false)
	...
	document.location = "/goform/ipFilterDeleteAll" + "?csrftoken=" + getCsrftoken();
}
...
/*add begin by 陈鹏 20180730 原因:增加IPV4过滤页面源地址、目的地址的输入校验 需求来源:龚忠锋*/
	if(!fw_checkIP2(document.getElementById("fw_srcIPStart"), document.getElementById("fw_srcIPEnd")))
	{
		return false;
	}
	
    if(!fw_checkIP2(document.getElementById("fw_dstIPStart"), document.getElementById("fw_dstIPEnd")))
	{
		return false;
	}
...
//				document.write('<TD align="center">' + srcIPStartArray[i] + '--'+ srcIPEndArray[i] + '</TD>');
				if(html_decode(srcIPStartArray[i]) == " ")
				{
					document.write('<TD align="center">' + '--' + '</TD>');
				}
				else if(html_decode(srcIPEndArray[i]) == " ")
				{
...
//				document.write('<TD align="center">' + dstIPStartArray[i] + '--'+ dstIPEndArray[i] + '</TD>');
				if(html_decode(dstIPStartArray[i]) == " ")
				{
					document.write('<TD align="center">' + '--' + '</TD>');
				}
				else if(html_decode(dstIPEndArray[i]) == " ")
				{
